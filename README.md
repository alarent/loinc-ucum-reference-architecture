...
Эмпирически разрыв подтверждается типовым ландшафтом российского healthtech: крупные платформы строят интеграцию через внутренние «абстрактные справочники биомаркеров» без UCUM-канонизации и version-aware mapping. Это типовая картина.
## До и после
...
- интеграторы EHR-систем и команд healthcare-платформ
## Как использовать
